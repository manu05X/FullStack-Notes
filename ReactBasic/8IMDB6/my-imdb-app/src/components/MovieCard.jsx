...
function MovieCard({ movieObject }) {
  ...
  let myMovieContaxt = useContext(MovieContext);

  //console.log(myMovieContaxt.watchlist[0].id);
  ...
  return (
    ...
    >
      {/* heart eye emoji
       onClick={handleAddtoWatchListProps(movieObject)} ->  if we donot use ()=> the this will run till all the data is added. So we use function so it is triggered or calle donly whent imoji
...
function MovieCard({ moviesObject }) {
  // let myMovieContext = useContext(MovieContext);
  //console.log("From MovieCard " + moviesObject.id);

  let { handleAddtoWatchList, WatchList, DeleteFromWatchList } =
    useContext(MovieContext);

  function doesContain() {
    // console.log(moviesObject.id);
  ...
  return (
    <div
      className="h-[40vh] w-[200px] bg-cover flex items-end rounded-lg hover:scale-110 duration-300"
      style={{
        backgroundImage: `url(https://image.tmdb.org/t/p/original/${moviesObject.poster_path})`,
      }}
    >
      // heart eye emoji
      //  onClick={handleAddtoWatchListProps(movieObject)} ->  if we donot use ()=> the this will run till all the data is added. So we use function so it is triggered or calle donly whent imoji
      ...
      {doesContain(moviesObject) ? (
        <div
          onClick={() => DeleteFromWatchList(moviesObject)}
          className="m-4 flex justify-center h-8 w-8 items-center rounded-lg bg-gray-900/60"
        >
          // code for cross 
          &#10060;
